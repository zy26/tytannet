...
namespace Pretorianie.Tytan.Actions
{
    ...
    public sealed class ExtractPropertyRefactor : IPackageAction
    {
        ...
        private static string GenerateSourceCodeOutput(string codeClassName, IList<CodeVariable> vars, IList<string> varNames, IList<string> propNames, CodeModelLanguages language, PropertyGeneratorOptions options, string regionName)
        {
            // generate output:
            if (vars != null)
            {
