...
namespace Pretorianie.Tytan.Core.Helpers
{
    ...
    public static class NameHelper
    {
        ...
        public static void GetVariableNames(IList<CodeVariable> vars, out IList<string> varNames, out IList<string> propNames, CodeModelLanguages language)
        {
            ...
            else
            {
                ...
                foreach (CodeVariable v in vars)
                {
                    ...
                    if (string.Compare(propName, v.Name, language == CodeModelLanguages.VisualBasic) == 0)
                    {
                        do
                        {
                            varName = "_" + NameHelper.GetVariableName(v.Name, language);
                        }
                        while (string.Compare(propName, v.Name, true) != 0);
                    }
                    else varName = v.Name;

                    varName = GetParameterName(v.Name, language);
        ...
        public static IList<string> GetParameterNames(IList<CodeVariable> vars, CodeModelLanguages language)
        {
            ...
            foreach (CodeVariable v in vars)
            {
                name = NameHelper.GetParameterName(v.Name, language);
